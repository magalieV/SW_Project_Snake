...
class GameOver:
    def __init__(self, window, window_size):
        ...
        self.sound_effect = pygame.mixer.Sound(
            "game_module/assets/sound/over_sound.mp3")
        ...
        self._rank_back = pygame.Surface(
            (self._back_size[0], self._back_size[1]), pygame.SRCALPHA)
    ...
    def init_rect(self):
        self.rect_to_menu = Rect((self._window_size[0] / 6) * 2 - (self.text_to_menu.get_width(
        ) / 2), (self._window_size[1] / 4) * 3, self.text_to_menu.get_width(), self.text_to_menu.get_height())
        self.rect_quit = Rect((self._window_size[0] / 6) * 4 - (self.text_quit.get_width(
        ) / 2), (self._window_size[1] / 4) * 3, self.text_quit.get_width(), self.text_quit.get_height())
    ...
    def draw_text(self):
        self.screen.blit(
            self._rank_back, (self._window_size[0]/2 - self._back_size[0]/2, 0.10 * self._window_size[1]))
        self.screen.blit(self.text_game_over, (self._window_size[0] / 2 - (
            self.text_game_over.get_width() / 2), self._window_size[1] / 4))
        self.screen.blit(self._score, (self._window_size[0] / 2 - (
            self._score.get_width() / 2), self._window_size[1] / 2))
        self.screen.blit(self.text_to_menu, ((self._window_size[0] / 6) * 2 - (
            self.text_to_menu.get_width() / 2), ((self._window_size[1] / 4) * 3)))
        self.screen.blit(self.text_quit, ((self._window_size[0] / 6) * 4 - (
            self.text_quit.get_width() / 2), ((self._window_size[1] / 4) * 3)))
